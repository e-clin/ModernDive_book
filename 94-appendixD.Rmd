...
* I never understood this. If the pilot says "we're going to make up time in the air"
...
**Solution**: When datasets are in normal form, we can easily `_join` them with other datasets! For example, we can join the `flights` data with the `planes` data. Potential disadvantages may include increased complexity, performance issues, and challenges in  maintaining multiple datasets.
...
However, because the `flights` dataset contains `r unique(flights$tailnum) %>% length()` unique `tailnum`s, whereas the `planes` dataset only contains `r unique(planes$tailnum) %>% length()` unique `tailnum`s, some flights had missing `seats`s in the joined data frame, resulting in missing `ASM`s in the calculation. We can eliminate these by adding a `na.rm = TRUE` argument to `sum()`, telling R that we want to remove the `NA`'s in the sum. We saw this in Section \@ref(summarize):
...
While the above data frame is correct, the IATA `carrier` code is not always useful. For example, what carrier is `WN`? We can address this by joining with the `airlines` dataset using `carrier` as the key variable.
